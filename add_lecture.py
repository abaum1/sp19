...
intro_section = [
    {
        "type": "list",
        "name": "lecture_date",
        "message": "Please choose the lecture to update",
        "choices": [
            "{date}: {topic}".format(
                date=config[idx]["date"], topic=config[idx]["topic"]
            )
...
if answers['path_or_url'] == 'url':
    url_answer = prompt([{'type':'input', 'name': 'url', 'message': 'Enter the url'}])
    ...
    if not os.path.exists(path):
        ...
        sys.exit(1)
    dst = 'assets/lectures/lec{idx}/{filename}'.format(idx=str(idx), filename=os.path.split(path)[-1])
